const textArray = [
  ...
  function mainFn() {
    ...
    tarea.onkeydown = (e) => {
      ...
      if (e.key == text[pointer - 1]) {
        scoreval += 1;
      } else if (e.keyCode == 8 && pointer > 1) {
        if (scoreval > 0 && text[pointer - 2] == tarea.value[pointer - 2]) {
          scoreval -= 1;
        }
        spans[pointer - 2].style = "background-color:;color:#444444;";
      } else if (
        ...
        spans[pointer - 1].style =
          "background-color:#5d3123;color:#5d3123;";
      }
      ...
      if (start == 0) {
        ...
        timeInterval = setInterval(() => {
          time += 1;
          wpmsc.innerHTML =
            Math.floor((scoreval * 12) / Math.max(1, time)).toString() +
            " wpm";
          if (time > 60) {
            timer.innerHTML =
  ...
  function preventDefaultForScrollKeys(e) {
    if (e.keyCode == 13 || e.keyCode == 37 || e.keyCode == 46) {
      e.preventDefault();
      return false;
    }
  }
